...
# 마무리
### 이 프로젝트를 웹이나 모바일에서 실행이 가능하게 한다면 작은 매장에서 편히 움직이면서 재고를 조사할 수 있고, 
### 온라인으로 미리 주문을 해서 남아있는 재고 확인과 제품 리스트를 보면서 제품이 없을 때의 번거로움을 줄이기 위해 필요하다고 생각한다.
### 아쉬운 점은 직접 이 프로젝트에서 웹과 모바일에서도 사용이 가능하게 했다면 더 좋은 결과가 나왔을텐데 웹과 모바일에 관한 지식이 부족해서 구현해 내지 못한게 아쉬웠다.
